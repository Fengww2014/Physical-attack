...
class AttackLoss(torch.nn.Module):
    def __init__(self, target):
        ...
        self.model = models.inception_v3(pretrained=True).cuda()
        self.img_size = 320
        if target < 0:
            self.isTarget = False
    ...
    def Transformations(self, img_torch):
        ...
        def Fileter(img_torch):
            color_filter  = Variable((torch.eye(3)+0.05*torch.randn(3,3)).cuda())
            filted = torch.zeros(1,3,self.img_size,self.img_size).cuda()
            for k in range(3):
                ...
                tmp = processed_filter.repeat(1,1,self.img_size,self.img_size).mul(img_torch.unsqueeze(0))
                filted[:,k,:,:] = torch.sum(tmp, dim=1, keepdim=True)
            filted = filted.squeeze()
            return filted


        Img1 = Resize(img_torch, 0.9)
        Img2 = img_torch + torch.randn_like(img_torch) * 0.05
        Img3 = Fileter(img_torch)
        Img4 = Rotation(img_torch, 10*math.pi/180)
        Img5 = Rotation(img_torch, -10*math.pi/180)
        Img6 = Rotation(img_torch, 20*math.pi/180)
        Img7 = Rotation(img_torch, -20*math.pi/180)
        Img8 = Rotation(Img1, 10*math.pi/180)
        Img9 = Rotation(Img1, -10*math.pi/180)
        Img10 = Rotation(Img1, 20*math.pi/180)
        Img11 = Rotation(Img1, -20*math.pi/180)
        Img12 = Rotation(Img2, 10*math.pi/180)
        Img13 = Rotation(Img2, -10*math.pi/180)
        Img14 = Rotation(Img2, 20*math.pi/180)
        Img15 = Rotation(Img2, -20*math.pi/180)
        Img16 = Rotation(Img3, 10*math.pi/180)
        Img17 = Rotation(Img3, -10*math.pi/180)
        Img18 = Rotation(Img3, 20*math.pi/180)
        Img19 = Rotation(Img3, -20*math.pi/180)
        Img20 = Rotation(img_torch, 30*math.pi/180)
        Img21 = Rotation(img_torch, -30*math.pi/180)
        Img22 = Rotation(Img1, 30*math.pi/180)
        Img23 = Rotation(Img1, -30*math.pi/180)
        Img24 = Rotation(Img2, 30*math.pi/180)
        Img25 = Rotation(Img2, -30*math.pi/180)
        Img26 = Rotation(Img3, 30*math.pi/180)
        Img27 = Rotation(Img3, -30*math.pi/180)
        TransImgs = torch.stack((img_torch,Img1,Img2,Img3,Img4,Img5,Img6,Img7,Img8,Img9,Img10,Img11,Img12,Img13,Img14,Img15,Img16,Img17,Img18,Img19,Img20,Img21,Img22,Img23,Img24,Img25,Img26,Img27),dim = 0)
        return TransImgs
...
class AttackEPercepCycleGANModel(BaseModel):
    """
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_ATTACK_A', type=float, default=0.0, help='weight for ATTACK loss for adversarial attack in rec A')
        return parser
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'D_B', 'G_B', 'cycle_B', 'G_ATTACK', 'idt_B', 'percep']
    ...
    def forward(self):
        ...
        #self.noise = self.rec_A - self.real_A
    ...
    def backward_G(self):
        ...
        lambda_att_A = self.opt.lambda_ATTACK_A
        lambda_att_B = self.opt.lambda_ATTACK_B
        lambda_MI = self.opt.lambda_MI
        lambda_percep = self.opt.lambda_percep
        ...
        if lambda_att_A == 0:
            ...
            self.loss_G_ATTACK = self.criterionATTACK(self.fake_B)* lambda_att_B + self.criterionATTACK(self.rec_A)* lambda_att_A
        # GAN loss D_B(G_B(B))
        ...
        # MI loss
        if lambda_MI > 0:
            self.loss_MI = self.criterionMI(self.fake_B, self.real_A) * lambda_MI 
        else: 
            self.loss_MI = 0
        if lambda_percep > 0:
            self.loss_percep = self.criterionPercep(self.real_A, self.fake_B) * lambda_percep 
        else: 
            self.loss_percep = 0
        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B + self.loss_G_ATTACK + self.loss_MI + self.loss_percep
