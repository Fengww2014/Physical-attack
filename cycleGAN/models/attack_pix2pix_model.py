...
class AttackLoss(torch.nn.Module):
    def __init__(self):
        ...
        self.target_class = 286 #target label in imagenet
    ...
    def forward(self, Images): 
        ...
        for i in range(batch_size_cur):
            I = Images[i,:].squeeze()
            TransImgs = self.Transformations(I)
            output = self.model(TransImgs)
            ...
            #attackloss += (functional.softmax(output)[:, self.original_class]).mean()
            print('loss:', attackloss)
        attackloss = attackloss/batch_size_cur
        return attackloss


class AttackPix2PixModel(BaseModel):
    ...
    def backward_G(self):
        ...
        #ATTACK loss
        self.loss_G_ATTACK = self.criterionATTACK(self.fake_B)* self.opt.lambda_MI
        # combine loss and calculate gradients
        self.loss_G = self.loss_G_GAN + self.loss_G_L1 + self.loss_G_ATTACK
