...
class AttackLoss(torch.nn.Module):
    def __init__(self, target):
        ...
        self.model2 = models.alexnet(pretrained=True).cuda()
        if target < 0:
            self.isTarget = False
    ...
    def Transformations(self, img_torch):
        def get_affine_matrix(img_torch, rad=0.0, tx=0.0, ty=0.0, scale=1.0, a=0.0, b=0.0):
            # inverse
            scale = 1.0 / scale
            a = -a
            b = -b
            ...
            ty += 256*(1-scale)/2 
            theta = torch.tensor([
                [scale*np.cos(rad) - b*scale*np.sin(rad), -scale*np.sin(rad) + a*scale*np.cos(rad), tx],
                [scale*np.sin(rad) + b*scale*np.cos(rad), scale*np.cos(rad) + a*scale*np.sin(rad), ty]
        ...
        def rand(minval, maxval):
            return minval + (maxval-minval)*np.random.random()
            
        rad = rand(minval=-np.pi/4, maxval=np.pi/4)
        scale = rand(minval=0.5, maxval=2.0)
        a = rand(minval=-0.2, maxval=0.2)
        b = rand(minval=-0.2, maxval=0.2)
        tx = rand(minval=-30, maxval=30)
        ty = rand(minval=-30, maxval=30)

        Img0 = img_torch + torch.randn_like(img_torch) * 0.05
        Img1 = get_affine_matrix(img_torch,rad,scale,a,b,tx,ty) + torch.randn_like(img_torch) * 0.05
        Img2 = get_affine_matrix(img_torch,-rad,1/scale,-a,-b,-tx,-ty) + torch.randn_like(img_torch) * 0.05

        TransImgs = torch.stack((Img0,Img1,Img2),dim = 0)
        return TransImgs
    ...
    def forward(self, Images): 
        ...
        for i in range(batch_size_cur):
            ...
            if self.isTarget == True:
                ...
                attackloss2 -= (functional.softmax(output2)[:, self.target_class]).mean()
            else: 
                attackloss1 += (functional.softmax(output1)[:, self.target_class]).mean()
                attackloss2 += (functional.softmax(output2)[:, self.target_class]).mean()
            #print('loss:', attackloss)
        attackloss = (attackloss1 + attackloss2)/(2*batch_size_cur)
        return attackloss
...
class AttackEotGANModel(BaseModel):
    """
