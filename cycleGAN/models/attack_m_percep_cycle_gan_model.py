...
class AttackMPercepCycleGANModel(BaseModel):
    """
    ...
    def __init__(self, opt):
        ...
        if opt.target==0:
            self.target = random.choice([193,207])
        else:
        ...
        if self.isTrain:
            ...
            self.criterionATTACK = AttackLoss(self.target)
            self.criterionPercep = torch.nn.MSELoss()
    ...
    def forward(self):
        ...
        self.label_embedding = torch.nn.Embedding(2, 2)
        self.input_A = torch.cat((self.real_A.view(self.real_A.size(0), -1), self.label_embedding(self.target)), -1)
    ...
    def backward_G(self):
        ...
        # MI loss
        if lambda_MI > 0:
            self.loss_MI = self.criterionMI(self.rec_A, self.real_A) * lambda_MI 
        else: 
            self.loss_MI = 0
        if lambda_percep > 0:
